...
class OAuthHandler:
    ...
    def token(self, **kwargs):
        ...
        Args:
            **kwargs: Request parameters (via form or JSON)
        ...
        try:
            ...
            keys_str = (list(data.keys()) if hasattr(data, 'keys')
                        else 'not a dict')
            say(f"token() data keys: {keys_str}")
            ...
            # Debug logging
            say(f"token() parsed: grant_type={grant_type}, "
        ...
        if client_id != self.client_id or \
           client_secret != self.client_secret:
            say(f"Invalid client credentials: "
                f"client_id match={client_id == self.client_id}, "
                f"client_secret match="
